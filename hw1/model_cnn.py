...
from keras.layers import Dense, Activation, LSTM, TimeDistributed, Masking, Dropout, Bidirectional, GRU, BatchNormalization, Conv1D, MaxPooling1D, Conv2D
...
def main(argv):
	...
	index_1 = fbank_train_data[:,-3].argsort(kind='mergesort')
	#mfcc_train_data = mfcc_train_data[index_1]
	fbank_train_data = fbank_train_data[index_1]
	index_2 = fbank_train_data[:,-2].argsort(kind='mergesort')
	...
	count = 1
	for i in range(fbank_train_data.shape[0]-1):
		if fbank_train_data[i,-3] == fbank_train_data[i+1,-3] and fbank_train_data[i,-2] == fbank_train_data[i+1,-2]:
			count += 1
		else:
			frame_amount.append(count)
			count = 1
	...
	#fbank_train_data[:,0:23] = fbank_train_data[:,0:23]/10 
	...
	x, y, y_one_hot = batch_padding(fbank_train_data, label[:,1], one_hot_label, frame_amount, MAX_FRAME_LENGTH, 69)
	...
	model = Sequential()
	model.add(Conv1D(input_shape = (1000, 69), filters = 128, kernel_size = 2, strides=1, padding = 'same'))
	...
	model.fit(X_train, y_one_hot_train, epochs=120, batch_size=64, callbacks=[model_check, reduce_lr], validation_data=(X_test, y_one_hot_test))
