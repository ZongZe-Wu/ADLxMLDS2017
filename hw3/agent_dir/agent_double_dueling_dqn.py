...
class Agent_DDDQN(Agent):
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('output'):
                ...
                self.q_tar = target_value + (target_advantage - tf.reduce_mean(target_advantage, axis=1, keep_dims = True))
        ...
        with tf.variable_scope('q_target'):
            ...
            q_target = self.reward + self.gamma * (1 - self.terminal) * self.q_next
    ...
    def train(self):
        ...
        for i_episode in range(MAX_EPOCH):
            ...
            for t in range(MAX_STEPS):
                if i_episode % 100 == 0 and i_episode != 0:
                    ...
                    if ep_t % self.replace_target_iteration == 0:
                        ...
                        save_path = saver.save(self.sess, "Model_double_deuling/model_double_deuling_dqn.ckpt")
                if ep_t % 4 == 0 and ep_t > self.batch_size:
                    ...
                    q_next_from_eval, q_next_from_tar = self.sess.run([self.q_eval, self.q_tar], feed_dict={self.state : s_, self.state_ : s_})
                    choose_q = q_next_from_tar[np.arange(self.batch_size), np.argmax(q_next_from_eval, axis = -1)]
            ...
            np.save('dddqn_reward.npy', rewards)
