...
class Agent_PPO(Agent):
    ...
    def update(self, s, s_, a, r, t):
        ...
        for _ in range(A_UPDATE_STEPS):
            value_ = self.sess.run(self.value, feed_dict={self.state: s_})
            adv = self.sess.run(self.advantage, feed_dict={self.state: s, self.reward: r, self.value_:value_, self.terminal:t})
            hh, ttt = self.sess.run([self._actor_train_op, self.ttt], feed_dict={self.state: s, self.action: a, self.adv:adv})
            #print(ttt)
        # update critic net
        
        for _ in range(C_UPDATE_STEPS):
            value_ = self.sess.run(self.value, feed_dict={self.state: s_})
            self.sess.run(self._critic_train_op, feed_dict={self.state: s, self.reward: r, self.value_:value_, self.terminal:t})
    ...
    def _discount_and_norm_rewards(self):
        discounted_r = np.zeros_like(self.ep_rs)
        #s = s[np.newaxis, :]
        value = self.sess.run(self.value, {self.state: self.ep_obs_})
        #r = self.ep_rs[::-1]
        #t = self.ep_t[::-1]
        lastgaelam = 0
        ...
        for i_episode in range(MAX_EPOCH):
            ...
            for t in range(MAX_STEPS):
                ...
                if not testing and ((t+1) % self.batch_size == 0):
                    ...
                    discounted_ep_rs = self._discount_and_norm_rewards(self.ep_rs)
